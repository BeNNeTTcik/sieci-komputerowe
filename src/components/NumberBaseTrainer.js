...
import numberBasePool from './numberBasePool';
...
export default function NumberBaseTrainer({count = 10, title, storageKey}) {
  const key = `number-base-trainer:${storageKey || slugify(title)}`;
  const loadedRef = useRef(false);
  const fileInputRef = useRef(null);

  function drawNewSet() {
    return sample(numberBasePool, count);
  }

  function resolveProblemsByIds(ids) {
    const byId = new Map(numberBasePool.map(p => [p.id, p]));
  ...
  function exportProgress() {
    const data = {
      _typ: 'number-base-trainer-progress',
      _wersja: 1,
      title: title || null,
  ...
  function handleFileSelected(e) {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data._typ !== 'number-base-trainer-progress') {
  ...
  const givenBox = {
    ...
    padding: '18px 20px', marginBottom: '24px', textAlign: 'center',
  };
  const fieldLabel = { fontSize: '0.85rem', color: '#374151', marginBottom: '4px', display: 'block' };

  function inputStyle(flagged) {
    const base = {
      width: '100%', padding: '9px 12px', borderRadius: '8px', fontSize: '1rem',
      marginBottom: '4px', boxSizing: 'border-box', fontFamily: 'monospace',
    };
  ...
  if (phase === 'results') {
    return (
      <div style={card} className="printable-results">
        <h3 style={heading}>{title || 'Wynik — konwersja systemów liczbowych'}</h3>

        {transferBar}
        {importError && <div style={{color: '#dc2626', fontSize: '0.85rem', marginBottom: '14px'}}>⚠️ {importError}</div>}
        {importSuccess && <div style={{color: '#16a34a', fontSize: '0.85rem', marginBottom: '14px'}}>✅ Postęp wczytany z pliku.</div>}
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{fontWeight: 600, color: '#111827', marginBottom: '6px'}}>
                {ok ? '✅' : '❌'} Zadanie {i + 1} — {p.given[0].label}: <code>{p.given[0].value}</code>
              </div>
              {!ok && (
  ...
  return (
    <div style={card}>
      <h3 style={heading}>{title || 'Ćwiczenie — konwersja systemów liczbowych'}</h3>
      <p style={subheading}>Zadanie {current + 1} z {total}</p>
      ...
      <div style={givenBox}>
        <div style={{fontSize: '0.85rem', color: '#6b7280', marginBottom: '6px'}}>{p.given[0].label}</div>
        <div style={{fontSize: '2rem', fontWeight: 700, color: '#111827', fontFamily: 'monospace'}}>
          {p.given[0].value}
        </div>
      </div>
      ...
      {p.blanks.map((b) => {
        ...
        return (
          <div key={b.key} style={{marginBottom: '14px'}}>
            ...
            />
            {flagged && <div style={{fontSize: '0.8rem', color: '#dc2626'}}>To pole jest puste.</div>}
          </div>
        );
      })}
